...
abstract class ReusableMethods {
  ...
  static String prufeName(String value) {
    RegExp a =
    RegExp(r'^[A-Za-zöÖäÄüÜßàéëïáêèíîôóúûŠšŽžÅÕõÇÊÎŞÛçşĂÂŢăâţĳÆØæøåÔŴŶÁÉÍÏŵŷÓÚẂÝÀÈÌÒÙẀỲËẄŸẃýì'
...
abstract class Verwaltung{
  ...
  getMeineTermine() async {
    var response=await http.get('${Verwaltung.url}meine-termine',headers:{'token':'${Verwaltung.token}'});
...
//    var response = await http.get('${Verwaltung.url}/validate?token=${Verwaltung.token}');
//    // var response=await http.get('${Verwaltung.url}validate',headers:{'token': '${Verwaltung.token}'});
//    /*for debugging*/ print(response.statusCode);
//    if(response.statusCode == 200){
//      return true;
//    }else
...
class User{
  ...
  set email(String value) {
    ...
    RegExp abc = RegExp(r'[A-Za-z0-9!#\$%&+\/=?^_`{|}~-]+(?:.[A-Za-z0-9!#\$%&+\/=?^_`{|}~-]+)@(?:[a-zA-Z0-9]'
...
class Admin extends User{
  ...
  set anmeldung(String value){
    _anmeldung = ReusableMethods.prufeName(ReusableMethods.stringPrufung(value));
  }
...
class UserTermin{
  ...
  set beschreibung(String value){
    _beschreibung=ReusableMethods.prufeName(ReusableMethods.stringPrufung(value));
  }
